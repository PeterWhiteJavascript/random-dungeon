...
Quintus.Audio = function(Q) {
  ...
  Q.audio.enableWebAudioSound = function() {
    ...
    Q.audio.play = function(s,options) {
      ...
      source.assetName = s;
      source.startedAt = s.startAt || 0;
      if(source.start) { source.start(0, options.startAt || 0); } else { source.noteOn(0); }
      
      Q.audio.playingSounds[soundID] = source;
      
      
        //Volume stuff added by Peter
        // Create a gain node.
        source.gainNode = Q.audioContext.createGain();
        // Connect the source to the gain node.
        source.connect(source.gainNode);
        // Connect the gain node to the destination.
        source.gainNode.connect(Q.audioContext.destination);
        source.gainNode.gain.value = options.volume || 1;
    };
    Q.audio.changeVolume = function(s, value){
      for(var key in Q.audio.playingSounds) {
        var snd = Q.audio.playingSounds[key];
            snd.gainNode.gain.value = value;
      }
    };
    Q.audio.pause = function(s){
      for(var key in Q.audio.playingSounds) {
        var snd = Q.audio.playingSounds[key];
        if(!s || s === snd.assetName) {
            Q.audio.pausedSounds[key] = snd;
            snd.pausedAt = snd.context.currentTime - snd.startedAt;
            Q.audio.stop(s);
            Q.audio.removeSound(key);
        }
      }  
    };
    Q.audio.resume = function(s){
        var played = false;
      for(var key in Q.audio.pausedSounds) {
        var snd = Q.audio.pausedSounds[key];
        if(!s || s === snd.assetName) {
            delete Q.audio.pausedSounds[key];
            Q.audio.removeSound(key);
            if(!played) Q.audio.play(s, {startAt:snd.pausedAt, volume:snd.gainNode.gain.value});
            played = true;
